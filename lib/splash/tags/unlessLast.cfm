...
The <s:unlessLast> checks to see if the currentChild is the last one in a children-each collection. If not, it 
passes the currentChild page to decendent tags as request.tags.currentChild.

If the currentChild is the last child, the tag terminates.
--->
...
<cfif thisTag.executionMode IS "start">
	...
	<cfif request.tags.loopIndex IS ArrayLen(request.tags.page.kids)>
		<cfexit method="exittag" />
	<cfelse>
